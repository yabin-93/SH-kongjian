...
test.describe('P0 - P2 users', () => {
  test.describe.configure({ mode: 'serial' });
  test.beforeEach(() => test.skip(!env.credentials, 'Admin credentials are required.'));

  test('TC-P2U-001 user table loads', async ({ authenticatedPage }) => {
    const users = new P2UsersPage(authenticatedPage);
    await users.goto();

    await expect(users.table).toBeVisible();
  });

  test('TC-P2U-003 empty account shows its required error', async ({
    authenticatedPage,
  }) => {
  ...
  test('TC-P2U-004 empty password shows its required error', async ({
    authenticatedPage,
  }) => {
    ...
    await users.goto();
    const dialog = await users.openCreateDialog();
    await users.accountField(dialog).fill(uniqueName('p2-required'));
    await users.submitDialog(dialog);

    await expect(users.errorByLabel(dialog, '用户密码')).toBeVisible();
  });

  test('TC-P2U-001 creates a unique user when mutations are enabled', async ({
    authenticatedPage,
  }) => {
    test.skip(!mutationEnabled(), mutationSkipReason);
    const users = new P2UsersPage(authenticatedPage);
    const account = uniqueName('pw-p2');
    await users.goto();
    const dialog = await users.openCreateDialog();
